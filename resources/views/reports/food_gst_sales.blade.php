...
<head>
    <meta charset="utf-8">
    <title>Food / GST sales (lines)</title>
    <style>
    ...
    <table>
        <thead>
            <tr>
                @foreach($headers as $idx => $h)
                    @php
                        $isNumeric = $idx >= 1 && !in_array($h, ['Line ID', 'Line status', 'Bill #', 'Customer', 'GSTIN', 'Business date', 'Item', 'Tax type', 'Payment', 'Order status', 'Closed / voided', 'Waiter', 'Cashier']);
                    @endphp
        ...
        <tbody>
            @php
                $numericHeaders = [];
                foreach($headers as $idx => $h) {
                    $numericHeaders[$idx] = $idx >= 1 && !in_array($h, ['Line ID', 'Line status', 'Bill #', 'Customer', 'GSTIN', 'Business date', 'Item', 'Tax type', 'Payment', 'Order status', 'Closed / voided', 'Waiter', 'Cashier']);
                }
